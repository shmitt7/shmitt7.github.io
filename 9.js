(function () {
    ...
    document.head.insertAdjacentHTML('beforeend', '<style>' +
        ...
        /* Иконки — правый верх, без фона, объёмные */
        ...
        /* Оверлей: меньше боковых отступов (п.3), ниже (п.2) */
        '.card__overlay{position:absolute;left:0;right:0;bottom:0;' +
            'padding:3em 0.4em 0.25em 0.4em;' +   /* боковые 0.4em вместо 0.7em; низ 0.25em вместо 0.6em */
        ...
        /* Строка статуса — меньше отступ снизу (п.4), обрезка в середине (п.6) */
        '.card__status-row{display:flex;align-items:baseline;margin-bottom:0.15em;' +   /* было 0.3em */
            'line-height:1;overflow:hidden;white-space:nowrap;min-width:0}' +
        '.card__status-row:empty{display:none}' +
        '.card__status-row .tvs-icon{font-size:1.1em;margin-right:0.2em;flex-shrink:0}' +
        /* tvs-main обрезается с ellipsis, tvs-suffix (год/дата) всегда виден */
        '.card__status-row .tvs-main{font-size:0.85em;color:#ccc;' +   /* #ccc вместо #aaa (п.1) */
        ...
        '.card__badge-year,.card__badge-sep{font-size:0.85em;color:#ccc;flex-shrink:0;white-space:nowrap}' +   /* #ccc (п.1) */
        '.card__badge-genre{font-size:0.85em;color:#ccc;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0}' +   /* #ccc (п.1) */
        '.card__badge-quality,.card__badge-rating{font-size:1em;font-weight:700;color:#ddd;flex-shrink:0;white-space:nowrap}' +
        /* Иконка KP: чуть больше и ближе к цифрам (п.5) */
        '.card__badge-kp-icon{display:inline-block;width:1em;height:1.25em;' +   /* было 0.8em/1em */
            'background-repeat:no-repeat;background-position:center;background-size:contain;' +
            'margin-left:0.05em;vertical-align:middle;flex-shrink:0}' +   /* было 0.1em */
    '</style>');
    ...
    function getTVInfo(info) {
        ...
        if (!last) {
            ...
            if (st === 'Planned')          { var y = fmtYear(info.first_air_date); return { text: 'Запланировано' + (y ? ' ' + y : ''), icon: '❱', color: '#E040FB' }; }
            return null;
        }
        ...
        if (st === 'In Production') return { text: fmtPremiere(info.first_air_date), icon: '✦', color: '#E040FB' };
        if (st === 'Planned')       { var y2 = fmtYear(info.first_air_date); return { text: 'Запланировано' + (y2 ? ' ' + y2 : ''), icon: '❱', color: '#E040FB' }; }
        return { text: ep, icon: '⏯︎', color: '#40C4FF' };
    }

    function getMovieInfo(info) {
        var st = info.status, rd = info.release_date;
        if (st === 'Rumored')         { var y = fmtYear(rd); return { text: 'По слухам' + (y ? ' ' + y : ''), icon: '❱', color: '#E040FB' }; }
        if (st === 'Planned')         { var y2 = fmtYear(rd); return { text: 'Запланировано' + (y2 ? ' ' + y2 : ''), icon: '❱', color: '#E040FB' }; }
    ...
    // Разбивает текст статуса на основную часть и суффикс (год/дата)
    // чтобы обрезка шла по основному слову, а суффикс всегда был виден (п.6)
    function splitStatusText(text) {
        var parts = text.split(' ');
        if (parts.length < 2) return { main: text, suffix: '' };
        var last = parts[parts.length - 1];
        // Суффикс: год (2028), дата (16.12.26), "Xдн."
    ...
    function loadCardStatus(data, statusRow) {
        ...
        net.silent(
            ...
            function (resp) {
                ...
                var split = splitStatusText(li.text);
