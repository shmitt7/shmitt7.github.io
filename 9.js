(function () {  
    ...
    document.head.insertAdjacentHTML('beforeend', '<style>' +  
        ...
        '.crl-title{font-size:1.44em;line-height:1.2;font-weight:700;color:#fff;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;line-clamp:2;-webkit-box-orient:vertical;text-overflow:ellipsis;margin-bottom:0.2em;text-shadow:0 1px 3px rgba(0,0,0,0.9)}' +  
        '.crl-row{display:flex;justify-content:space-between;align-items:baseline;line-height:1.2;margin-top:0.18em}' +  
        ...
        '.crl-right{font-size:0.65em;color:rgba(255,255,255,0.92);white-space:nowrap;margin-left:0.5em;flex-shrink:0;display:flex;align-items:baseline;gap:0.3em}' +  
        '.crl-quality{background:#ffe216;color:#000;border-radius:0.25em;padding:0.05em 0.3em;font-weight:700;text-transform:uppercase;font-size:0.95em}' +  
        '.crl-vote{font-weight:700;color:#fff;display:flex;align-items:center;font-size:1.05em}' +  
        ...
        '.crl-status-icon{margin-right:0.2em}' +  
    '</style>');  
    ...
    function buildOverlay(card, data) {  
        var isTv = !!data.original_name;  
        var genreLabels = getGenreLabels(data, 2);  
        ...
        /* row 2: genre+type | quality+rating */  
        ...
        if (isTv) {  
            left2.textContent = genreLabels.length ? 'TV · ' + genreLabels.join(' · ') : 'TV';  
        } else {  
            left2.textContent = genreLabels.join(' · ');  
        }  
        ...
        left3.appendChild(statusIconEl);  
        left3.appendChild(statusTextEl);  
