(function () {
    ...
    document.head.insertAdjacentHTML('beforeend', '<style>' +
        ...
        '.crl-vote{display:flex;align-items:center;margin-left:0.3em}' +
    ...
    function getGenreLabels(data, max) {
        ...
        var ids;
        if (genreObjs.length) {
            ids = genreObjs.map(function(genre) { return typeof genre === 'object' ? genre.id : genre; });
        } else {
            ids = data.genre_ids || [];
        }
        var priority = null;
        ...
        function next() {
            ...
            qualityNetwork.silent(url, function(res) {
                ...
                for (var ri = 0; ri < results.length; ri++) {
                    ...
                    var year = parseInt((result.info && result.info.released) || result.year);
                    var inTitle = !targetYear || (result.Title && (result.Title.indexOf(String(targetYear)) !== -1 || result.Title.indexOf(String(targetYear - 1)) !== -1 || result.Title.indexOf(String(targetYear + 1)) !== -1));
                    if ((year && Math.abs(year - targetYear) <= 1) || (!year && inTitle)) titles.push(result.Title);
                }
                serverIndex++; next();
        ...
        function searchByTitle() {
            ...
            ratingNetwork.silent('https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=' + encodeURIComponent(query),
                function(json) {
                    ...
                    var best = null;
                    if (year) {
                        best = films.find(function(film) { return parseInt((film.year || '').slice(0, 4)) === year; });
                        if (!best) best = films.find(function(film) { var filmYear = parseInt((film.year || '').slice(0, 4)); return filmYear && filmYear > year - 3 && filmYear < year + 3; });
                    }
                    best = best || films[0];
    ...
    function buildEpisodeText(info) {
        ...
        for (var i = 0; i < seasons.length; i++) {
            ...
            if (season.season_number > 0 && season.season_number < currentSeason) airedTotal += season.episode_count;
        }
    ...
    function fetchStatus(data, callback) {
        ...
        var statusNetwork = new Lampa.Reguest();
        if (isTV) {
            statusNetwork.silent(
    ...
    function buildOverlay(card, data) {
        ...
        qualityEl.className = 'crl-quality-text';
        qualityEl.style.display = 'none';
        metaRight.appendChild(qualityEl);
        card._crlQualityEl = qualityEl;
    ...
    function updateCardAsync(card) {
        ...
        fetchQuality(data, function(quality) {
            if (!quality || !card._crlQualityEl) return;
            card._crlQualityEl.textContent = quality;
