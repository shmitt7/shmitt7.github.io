(function() {
    ...
    function buildReactionsEl(reactionsData) {
        ...
        const el = $('<span class="fsc-serial-badge fsc-reactions-corner"></span>');
        el.html(
    ...
    function init() {
        ...
        Lampa.Listener.follow('full', function(e) {
            ...
            setTimeout(function() {
                ...
                let currentKP = null;
                let currentQuality = '';
                if (movie.release_date || movie.first_air_date) year = (movie.release_date || movie.first_air_date).slice(0, 4);
                if (movie.runtime) runtime = Lampa.Utils.secondsToTimeHuman(movie.runtime * 60);
                else if (movie.episode_run_time && movie.episode_run_time.length) runtime = Lampa.Utils.secondsToTimeHuman(movie.episode_run_time[0] * 60);
                if (movie.production_countries && movie.production_countries.length) countries = movie.production_countries.slice(0, 2).map(function(c) { return c.iso_3166_1; });
                genreLabels = getGenreLabels(movie, 2);
                if (movie.vote_average) tmdbRating = parseFloat(movie.vote_average.toFixed(1));
                ...
                function checkKP(attempt) {
                    ...
                    if (kpEl.length) {
                        ...
                        if (!isNaN(val) && val >= 1) { currentKP = val; rebuildInfo(); return; }
                    }
                ...
                function checkQual(attempt) {
                    if (currentToken !== token || attempt > 30) return;
                    const qualEl = render.find('.full-start-new__quality');
                    if (qualEl.length) {
                        const val = qualEl.text().trim();
                        if (val) { currentQuality = val; rebuildInfo(); return; }
                    }
                    setTimeout(function() { checkQual(attempt + 1); }, 500);
                }
                checkQual(0);
                let serialEl = null;
                if (movie.first_air_date) {
                    ...
                    const serialParts = [];
                    if (tvStatus && !(tvStatus === 'Returning Series' && hasNextEpisode)) serialParts.push(Lampa.Lang.translate('tv_status_' + tvStatus.toLowerCase().replace(/ /g, '_')));
                    if (totalSeasons > 0) serialParts.push(Lampa.Lang.translate('title_seasons') + ': ' + (currentSeason > 0 && currentSeason < totalSeasons ? currentSeason + '/' + totalSeasons : totalSeasons));
                    if (totalEpisodes > 0) serialParts.push(Lampa.Lang.translate('title_episodes') + ': ' + (airedTotal > 0 && airedTotal < totalEpisodes ? airedTotal + '/' + totalEpisodes : totalEpisodes));
                ...
                right.find('.fsc-reactions-corner').remove();
