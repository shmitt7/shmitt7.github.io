(function(){  
    ...
    function tmdbSearch(method,query,cb,errcb){  
        if(!query){cb([]);return;}  
        var url='https://api.themoviedb.org/3/search/'+method+'?query='+encodeURIComponent(query)+'&language=ru-RU';  
        Lampa.TMDB.get(url,{},function(json){  
            cb(json&&json.results?json.results:[]);  
        },errcb,{life:60*24*7});  
    }  
    function resolveTmdbByCard(card,cb){  
        var cacheKey=card.method+'_'+card.kp_query_original+'_'+card.kp_year;  
        ...
        function finish(best){  
            var tmdbCard=null;  
            if(best){  
                tmdbCard=Lampa.Utils.addSource(best,'tmdb');  
                tmdbCard.method=card.method;  
                tmdbCard.kp_rating=card.kp_rating;  
            }  
            setResolveCache(cacheKey,tmdbCard);  
            cb(tmdbCard);  
        }  
    }  
    ...
    function loadCollectionResolved(type,page,oncomplite,onerror){  
        loadKpCollection(type,page,function(data){  
            var kpCards=data.items.map(mapKpCard);  
            resolveTmdbByList(kpCards,function(tmdbCards){  
                oncomplite({results:tmdbCards,total_pages:data.total_pages,total_results:data.total_results});  
            });  
        },onerror);  
    }  
