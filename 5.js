(function() {  
    ...
    async function fetchQuality(data, callback) {  
        const key = data.id;  
        ...
        for (const server of SERVERS) {  
            try {  
                const json = await fetch(server + '/api/v2.0/indexers/all/results?apikey=&Query=' + encodeURIComponent(title) + (targetYear ? '&year=' + targetYear : '')).then(r => r.json());  
