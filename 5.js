(function() {
    ...
    function init() {
        ...
        Lampa.Listener.follow('full', function(e) {
            ...
            $('body').find('.fsc-reactions-corner').remove();
            ...
            setTimeout(function() {
                ...
                const movieKey = mediaType + '_' + movie.id;
                if (Object.keys(reactionsCache).length > 50) reactionsCache = {};
                reactionsCache[movieKey] = e.data && e.data.reactions;
                const reactEl = buildReactionsEl(reactionsCache[movieKey]);
                if (reactEl) $('body').append(reactEl);
                if (movie.id) {
        ...
        Lampa.Listener.follow('activity', function(e) {
            if (e.type === 'archive' && e.component === 'full') {
                ...
                $('body').find('.fsc-reactions-corner').remove();
                const archiveCard = e.object.card;
                if (archiveCard && archiveCard.id) {
                    const archiveType = archiveCard.name ? 'tv' : 'movie';
                    const reactEl = buildReactionsEl(reactionsCache[archiveType + '_' + archiveCard.id]);
                    if (reactEl) $('body').append(reactEl);
                }
            }
