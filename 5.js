(function () {
    ...
    style.textContent = [
        ...
        // :not(.light--version) — в light-версии меню всегда видимо, оверлей не нужен
        'body.menu--open:not(.light--version) .wrap__left{left:1em!important;}',
        // wrap__content не сдвигается (меню — оверлей поверх контента)
        ...
        // ── РЕЖИМ menu--always ──────────────────────────────────────────────
        // Меню всегда видимо (5em), контент смещается через margin-left
        // (transform не работает, т.к. wrap__left вырван из потока через position:fixed)
        ...
        // При открытии в menu--always: меню расширяется до 15em, контент сдвигается
        ...
        // Исправлено: было rgba(0,0,0,0.85) без blur — это цвет плеера, не панелей
        ...
        // ── ПРАВЫЕ ПАНЕЛИ (settings, selectbox) ────────────────────────────
        ...
        // Исправлено: было rgba(0,0,0,0.85) без blur
        ...
        // ── ЦЕНТРАЛЬНЫЕ МЕНЮ (modal) ────────────────────────────────────────
        // border-radius уже есть в оригинале (1em), добавляем только border и фон
