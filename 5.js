...
if(!card.kp_source){  
Lampa.Activity.push({url:card.id,title:card.title||card.name,component:'full',source:'tmdb',method:card.method,id:card.id});  
return;  
}  
...
Lampa.Loading.stop();  
if(tmdbCard){  
Lampa.Activity.push({url:tmdbCard.id,title:tmdbCard.title||tmdbCard.name,component:'full',source:'tmdb',method:tmdbCard.method,id:tmdbCard.id});  
}  
else{  
...
function initPlugin(){  
var manifest={type:'video',version:'3.0.0',name:'Кинопоиск',description:'Популярное с Кинопоиска',component:'kinopoisk_main'};  
